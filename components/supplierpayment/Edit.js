...
const Edit = ({ message, id, data }) => {
    ...
    const showEditForm = async () => {
        ...
        try {
            ...
            const { dt, supplierId, rupee, taka } = data.find(supplierpayment => supplierpayment._id === id) || { dt: '', supplierId: '', rupee: '', taka: '' };
    ...
    const saveHandler = async (e) => {
        ...
        try {
            ...
            const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/supplierpayment/${id}`;
            ...
            if (response.ok) {
                ...
                throw new Error("Failed to create supplierpayment");
            }
        } catch (error) {
            console.error("Error saving supplierpayment data:", error);
            message("Error saving supplierpayment data.");
        } finally {
            setShow(false);
        }
    }
                            ...
                            <form onSubmit={saveHandler} >
                                <div className="grid grid-cols-1 gap-4 my-4">
                                    ...
                                    <DropdownEn Title="Supplier" Id="supplierId" Change={e => setSupplierId(e.target.value)} Value={supplierId}>
                                        {suppliers.length ? suppliers.map(supplier => <option value={supplier._id} key={supplier._id}>{supplier._id}</option>) : null}
                                    </DropdownEn>
