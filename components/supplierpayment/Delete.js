    ...
    const Delete = ({ message, id, data }) => {
        ...
        const showDeleteForm = () => {
            setShow(true);
            const { supplierId } = data.find(supplierpayment => supplierpayment._id === id) || { supplierId: "" };
            setDt(supplierId.name); 
        }
        ...
        const softDeleteHandler = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/supplierpayment/${id}`, {
                    method: "PATCH",
        ...
        const hardDeleteHandler = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error("Failed to delete supplierpayment");
                }         
            } catch (error) {
                console.log(error);
