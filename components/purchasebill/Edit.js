...
import { BtnSubmit, TextDt, TextNum } from "@/components/Form";
...
const Edit = ({ message, id, data }) => {
    ...
    const showEditForm = () => {
        setShow(true);
        const { dt, shipment, bale, meter, weight, taka } = data.find(purchasebill => purchasebill._id === id) || { dt: '', shipment: '', bale: '', meter: '', weight: '', taka: '' };
    ...
    const saveHandler = async (e) => {
        ...
        try {
            ...
            const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/purchasebill/${id}`;
            ...
            if (response.ok) {
                ...
                throw new Error("Failed to create purchasebill");
            }
        } catch (error) {
            console.error("Error saving purchasebill data:", error);
            message("Error saving purchasebill data.");
        } finally {
            setShow(false);
        }
    }
