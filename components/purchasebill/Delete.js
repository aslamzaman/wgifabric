    ...
    const Delete = ({ message, id, data }) => {
        ...
        const showDeleteForm = () => {
            setShow(true);
            const { shipment } = data.find(purchasebill => purchasebill._id === id) || { shipment: "" };
            setDt(shipment); 
        }
        ...
        const softDeleteHandler = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/purchasebill/${id}`, {
                    method: "PATCH",
        ...
        const hardDeleteHandler = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error("Failed to delete purchasebill");
                }         
            } catch (error) {
                console.log(error);
