...
const Edit = ({ message, id, data }) => {
    ...
    try {
        ...
        const { dt, customerId, cashtypeId, taka } = data.find(customerpayment => customerpayment._id === id) || { dt: '', customerId: '', cashtypeId: '', taka: '' };
...
const saveHandler = async (e) => {
    ...
    try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/customerpayment/${id}`;
        ...
        if (response.ok) {
            ...
            throw new Error("Failed to create customerpayment");
        }
    } catch (error) {
        console.error("Error saving customerpayment data:", error);
        message("Error saving customerpayment data.");
    } finally {
        setShow(false);
    }
}
