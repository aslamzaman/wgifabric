    ...
    const Delete = ({ message, id, data }) => {
        ...
        const showDeleteForm = () => {
            setShow(true);
            const { customerId } = data.find(customerpayment => customerpayment._id === id) || { customerId: "" };
            setDt(customerId.name); 
        }
        ...
        const softDeleteHandler = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/customerpayment/${id}`, {
                    method: "PATCH",
        ...
        const hardDeleteHandler = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error("Failed to delete customerpayment");
                }         
            } catch (error) {
                console.log(error);
